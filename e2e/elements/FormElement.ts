...
> extends GenericElement {
  ...
  constructor(
    ...
    protected elementsConstructors: ValueElementsConstructors<Values>,
    protected positive: boolean = true,
  ) {
    super(page, locator, positive);
    this.submit = new ButtonElement(
      this.page,
      this.locator.locator("[type=submit]"),
    );
    ...
    this.inputs = keys
      .map((key) => {
        return {
          [key]: elementsConstructors[key](
            page,
            locator.locator(`[name=${String(key)}]`),
          ),
        };
      })
      .reduce((a, b) => ({ ...a, ...b }), {});
