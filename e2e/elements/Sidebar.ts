...
class SidebarBlock extends GenericElement {
    get create() {
        return new ButtonElement(this.page, this.page.getByTestId("create"), this.positive)
    }
...
class SidebarTeamBlock extends SidebarBlock {
    get(name: string) {
        return new SidebarTeamEntry(this.page, this.locator.locator("[data-sidebar=menu]", {hasText: name}))
    }
}
